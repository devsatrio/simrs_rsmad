...
def GenerateNomorAntrian(poli):
    last_number = registrasiPasien.objects.filter(poliklinik_id=poli).filter(tgl_registrasi=date.today()).aggregate(Max('no_registrasi'))
...
def GenerateNomorRawat():
    today = date.today()
    last_number = registrasiPasien.objects.filter(tgl_registrasi=date.today()).aggregate(Max('no_rawat'))
...
class registrasiPasien(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        super(registrasiPasien, self).save(*args, **kwargs)